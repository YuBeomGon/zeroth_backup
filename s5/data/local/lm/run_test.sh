...
if [ ! -f $srcdir/rawCorpus.ra ] ; then
    echo " shuffle the raw data"
    shuf -n `cat $srcdir/rawCorpus.1 | wc -l` $srcdir/rawCorpus.1 > $srcdir/rawCorpus.ra
fi
...
if [ ! -e $srcdir/normedCorpus.1 ] ; then
    ...
    $cmd JOB=1:$nj $logdir/rawCorpus.JOB.log \
        $scriptdir/normStep1.py $srcdir/rawCorpus.ra \| \
...
#cp $srcdir/uniqWordList.hangul $srcdir/uniqWordList.hangul.pruned
wait
