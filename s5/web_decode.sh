...
decoder=~/kaldi/src/online2bin/online2-web-nnet3-decode-faster
echo "$0 $@"
...
orgsrcdir=test/models/korean/2nd_zeroth/
dir=test/models/korean/zeroth/decode/
...
if [ "$web_stage" -eq 3 ]; then
    ...
    $decoder $silence_weighting_opts \
          ...
          --verbose=2 \
          --min-active=$min_active --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam \
          --acoustic-scale=$acwt $srcdir/final.mdl $srcdir/HCLG.fst $srcdir/words.txt "dummy"
    exit 1
fi    

if [ "$web_stage" -eq 4 ]; then
    echo "web docoding for LM rescoring"
    $decoder $silence_weighting_opts \
          --frames-per-chunk=$frames_per_chunk \
          --extra-left-context-initial=$extra_left_context_initial \
          $frame_subsampling_opt \
          --config=$online_config \
          --verbose=2 --IsRescore=true \
          --min-active=$min_active --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam \
          --acoustic-scale=$acwt $orgsrcdir/final.mdl $orgsrcdir/HCLG.fst $orgsrcdir/words.txt $srcdir/G.carpa \
          $srcdir/words.txt
    exit 1
fi    
...
if $rescore ; then
	...
	lattice-lmrescore --lm-scale=-1.0 \
		...
		lattice-lmrescore-arpa --lm-scale=1.0 ark:- "$newlm" ark:- |\
